...
NODE *replace_alias_in_qualattr_list(NODE *alias, NODE *qualattr_list) {
  ...
  while (n) {
    ...
    if (s == NULL) { // one table in query
      ...
      cout << "One table in Q: " << alias->u.LIST.self->u.ALIAS.relname << endl;
    } else {
      cout << s << endl;
...
NODE *star_node(NODE *tables) {
  if (tables->u.LIST.next != NULL) {
    fprintf(stderr, "Error: must have only one table in the query\n");
    return NULL;
  }
  NODE *n = newnode(N_STAR);
  n->kind = N_STAR;
  n->u.QUALATTR.relname = tables->u.LIST.self->u.ALIAS.relname;
  return n;
}
